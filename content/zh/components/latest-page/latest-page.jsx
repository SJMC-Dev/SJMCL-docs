...
import {
  Box, Heading, Text, Spinner, HStack, VStack,
  Table, Thead, Tbody, Tr, Th, Td, Badge, Button, Link as CLink
...
export default function Latest() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        if (alive) setState({ loading: false, error: '获取最新版本失败', version: '', files: [] })
      }
    })()
    return () => { alive = false }
  }, [])
  ...
  if (state.loading) {
    return <HStack><Spinner /><Text>正在获取最新版本…</Text></HStack>
  }
  if (state.error) {
